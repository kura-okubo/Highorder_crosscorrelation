...
ifcheckRicker = true #debug Ricker wavelet
ifsourcenoise = true #add noise to source
noiselevel = 5 #[%] perturbation in frequency domain
noiseseed=1010:10:1300
fM = 0.5 # [Hz] peak (center) frequency

Figdirroot = "/Users/kurama/Documents/kurama/research/Highorder_crosscorrelation/code/fig_withsourcenoise/"
Filetype = "pdf"
#------------------------------------------------#
...
p_SNR_cc1 = plot(mean(cc1_12_SNanalysis, dims=1)', t_cc, line=(:black, 1, :solid),
    xlabel = "CC1:1-2 SNR ="*@sprintf("avg. %4.2f max. %4.2f", cc1_SNR_mean, cc1_SNR_max), 
    ylabel = "Lag time [s]",
    xlim = (-1.5*xplotrange_cc1, 1.5*xplotrange_cc1),
    ylim = (minplotT, maxplotT),
    size = (400, 800),
    xticks = []
    )

p_SNR_cc2 = plot(mean(cc2_12_SNanalysis, dims=1)', t_cc, line=(:black, 1, :solid),
...
plot(p_SNR_cc1, p_SNR_cc2, layout = layout, yflip = true, size = (800, 800), legend=false)
