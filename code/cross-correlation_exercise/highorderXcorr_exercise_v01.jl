...
Q = 1e4 	# Attenuation term (quality factor) Q
dt = 0.02
T = 100
NumofSource = 10;

ifRicker = false# If Ricker wavelet is convolved
...
return The Green’s function solution for two-dimensional space, under the far field approximation
"""
function gf(x::Array{Float64,1},xi::Array{Float64,1},omega::Float64, vel::Float64, rho::Float64, Q::Float64)
    r = norm(x.-xi)
    #g1 = -im * (1/(4*rho*v^2)) * sqrt((2*v)/(pi*omega*r)) * exp(-im*(omega/v)*r) * exp(-(omega*r)/(2*v*Q)) * exp(im*pi/4)
    g1 = (1/sqrt((8*pi*omega*(1/vel) * r))) * exp(-im * (omega* (1/vel) * r + pi/4)) * exp(-(omega* r)/(2*v*Q)) 
    #g1 = (1/sqrt((8*pi*omega*(1/vel) * r))) * exp(-im * (omega* (1/vel) * r + pi/4)) 
    #println(g1)
    return g1
end
...
"""
rickerWavelet(omega)
return zero-phase Ricker wavelets
"""
...
for i = 1:NumofSource
    theta = 2*pi*(i-1)/NumofSource
    Sx[i,:] = R .* [cos(theta), sin(theta)]  
end
...
NT = round(Int64, T/dt)
NumofFreq = NT
#omega = range(Freqmin, stop=Freqmax,length=NumofFreq)
fs = 1/dt
...
# take inverse fft
u1test = ifft(gf1[1,:])
#u1test = fftshift(u1test)

# return u1test[-maxlag:maxlag]
